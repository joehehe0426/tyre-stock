#!/usr/bin/env python3
"""Write clean main.dart - no PinGate, no null issues."""
import os
...
class _StockPageState extends State<StockPage> {
  ...
  Future<void> _parseXlsx(List<int> bytes) async {
    ...
    var sh = excel.tables['tyre_stock'] ?? excel.tables['工作表1'];
    if (sh == null && excel.tables.values.isNotEmpty) sh = excel.tables.values.first;
    if (sh == null || sh.rows.length < 2) throw 'no data';
    _all.clear();
    final hdr = sh.rows[0];
    final isNew = hdr.isNotEmpty && (hdr[1]?.value?.toString() ?? '').contains('Size');
    for (int r = 1; r < sh.rows.length; r++) {
      final row = sh.rows[r];
      if (isNew) {
        final ss = (row[1]?.value?.toString() ?? '').replaceAll(' ', '');
        ...
        if (w == 0 && a == 0) continue;
        final brand = (row[2]?.value?.toString() ?? '').trim();
        final desc = (row[3]?.value?.toString() ?? '').trim();
        String pt = desc;
        if (pt.toUpperCase().startsWith(brand.toUpperCase())) pt = pt.substring(brand.length).trim();
        if (pt.isEmpty) pt = desc;
        final price = double.tryParse(row[5]?.value?.toString() ?? '0') ?? 0;
        _all.add({'br': brand, 'pt': pt, 'w': w, 'a': a, 'ri': ri, 'bp': price * 0.6, 'sp': price, 'st': 1, 'su': '', 'de': desc});
      } else {
...
class _FitmentPageState extends State<FitmentPage> {
  ...
  List<Map<String, dynamic>>? _gens() {
    ...
    try {
      ...
      return (raw as List).cast<Map<String, dynamic>>().where((g) {
        final n = (g['n'] ?? g['name'] ?? '').toString().trim();
        return n.isNotEmpty && !n.startsWith('---');
      }).map((g) => {
        'name': g['n'] ?? g['name'] ?? '', 'year': g['y'] ?? g['year'] ?? '',
        'pcd': g['p'] ?? g['pcd'] ?? '', 'offset': g['o'] ?? g['offset'] ?? '',
        'cb': g['c'] ?? g['cb'] ?? '', 'thread': g['t'] ?? g['thread'] ?? '', 'torque': g['q'] ?? g['torque'] ?? '',
      }).toList();
    } catch (_) { return null; }
  }
  ...
  void _look() {
    final gens = _gens();
    if (_make == null || _model == null || _gen == null || gens == null || _gen! >= gens.length) return;
    final gen = gens[_gen!];
    final ml = _models;
    if (ml == null) return;
    try {
      final model = ml.firstWhere((m) => m['name'] == _model);
