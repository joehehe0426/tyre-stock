#!/usr/bin/env python3
"""Write the complete main.dart for tyre stock web app."""
import os
...
void main() => runApp(MaterialApp(
  ...
  theme: ThemeData(colorSchemeSeed: Color(0xFFD81B60), useMaterial3: true),
  home: PinGate(),
));
...
class _PinGateState extends State<PinGate> {
  ...
  Future<bool> _tryPin(String pin) async {
    ...
    if (stored == null) { await p.setString('pin', pin); await p.setBool('authed', true); return true; }
    if (pin == stored) { await p.setBool('authed', true); return true; }
    return false;
  }
  @override Widget build(BuildContext context) {
    if (_authed) return HomePage();
    return Scaffold(body: Center(child: _PinForm(onPin: (pin) async {
      final ok = await _tryPin(pin);
      if (ok && mounted) setState(() => _authed = true); return ok;
    })));
  }
}
...
class _PinFormState extends State<_PinForm> {
  final _c = TextEditingController(); bool _wrong = false;
  Future<void> _submit() async { final ok = await widget.onPin(_c.text); setState(() => _wrong = !ok); if (!ok) _c.clear(); }
  ...
  @override Widget build(BuildContext context) => Padding(padding: EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    SizedBox(height:16),Text('呔妹輪胎庫存',style:TextStyle(fontSize:22,fontWeight:FontWeight.bold)),
    SizedBox(height:8),Text('請輸入密碼',style:TextStyle(color:Colors.grey)),SizedBox(height:20),
    TextField(controller:_c,obscureText:true,autofocus:true,textAlign:TextAlign.center,
      style:TextStyle(fontSize:24,letterSpacing:8),
      decoration:InputDecoration(border:OutlineInputBorder(borderRadius:BorderRadius.circular(12)),isDense:true,contentPadding:EdgeInsets.symmetric(vertical:14,horizontal:16),errorText:_wrong?'密碼錯誤':null),
      onSubmitted:(_)=>_submit()),
    SizedBox(height:12),
    FilledButton.icon(onPressed:_submit,icon:Icon(Icons.login),label:Text('登入'),style:FilledButton.styleFrom(minimumSize:Size(double.infinity,48))),
    SizedBox(height:12),
    Text('🔑 首次使用: 輸入任意密碼即設定',style:Theme.of(context).textTheme.bodySmall?.copyWith(color:Colors.grey)),
  ]));
}
'''

# Write the rest of the file (HomePage, StockPage, FitmentPage)
# Using raw string concatenation to avoid truncation issues
code2 = '''
...
class _StockPageState extends State<StockPage> {
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      if (s != null && s.isNotEmpty) {
        ...
        if (!await _loadFromSheet() && !await _loadFromBundle()) _all = _fb();
      }
  ...
  void _pickFile() {
    ...
    input.onChange.listen((_) async {
      ...
      reader.onLoadEnd.listen((_) async {
        try {
          await _parseXlsx(reader.result as List<int>);
          await (await SharedPreferences.getInstance()).setString('d', jsonEncode(_all));
          _err = null; _reload();
          if (mounted) setState(() {});
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('已載入 ${_all.length} 款'), backgroundColor: Colors.green));
        } catch (e) {
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('檔案錯誤: $e'), backgroundColor: Colors.red));
        }
      });
    });
  }
    ...
    try {
      ...
      if (rows.length >= 2) {
        for (int r = 1; r < rows.length; r++) {
          ...
          _all.add({
            'br': cols[0].trim(), 'pt': cols[1].trim(),
            'w': int.tryParse(cols[2].trim())??0, 'a': int.tryParse(cols[3].trim())??0,
            'ri': int.tryParse(cols[4].trim())??0, 'bp': double.tryParse(cols[5].trim())??0,
            'sp': double.tryParse(cols[6].trim())??0, 'st': int.tryParse(cols[7].trim())??0,
            'su': cols.length>9?cols[9].trim():'', 'de': cols.length>10?cols[10].trim():'',
          });
        }
      }
      if (_all.isNotEmpty) { await (await SharedPreferences.getInstance()).setString('d', jsonEncode(_all)); _err = null; return true; }
    } catch (e) { _err = 'Google Sheet error: $e'; }
    return false;
  }

  Future<bool> _loadFromBundle() async {
    try { final d = await rootBundle.load('assets/stock.xlsx'); await _parseXlsx(d.buffer.asUint8List()); _err = null; return true; } catch (_) {}
    _err = '上傳 stock.xlsx 或設定 Google Sheet';
    return false;
  }

  Future<void> _parseXlsx(List<int> bytes) async {
    final sh = Excel.decodeBytes(bytes).tables['tyre_stock'];
    if (sh == null) throw 'no sheet';
    _all.clear();
    for (int r = 1; r < sh.rows.length; r++) {
      final row = sh.rows[r];
      if (row[0]?.value == null) continue;
      _all.add({
        'br': row[0]!.value.toString(), 'pt': row[1]!.value.toString(),
        'w': int.tryParse(row[2]?.value.toString()??'0')??0, 'a': int.tryParse(row[3]?.value.toString()??'0')??0,
        'ri': int.tryParse(row[4]?.value.toString()??'0')??0, 'bp': double.tryParse(row[5]?.value.toString()??'0')??0,
        'sp': double.tryParse(row[6]?.value.toString()??'0')??0, 'st': int.tryParse(row[7]?.value.toString()??'0')??0,
  ...
  Future<void> _setSheetUrl() async {
    ...
    final url = await showDialog<String>(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Google Sheet'), content: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('貼上 Google Sheet 分享連結'), SizedBox(height:8),
        TextField(controller: ctrl, decoration: const InputDecoration(hintText: 'https://docs.google.com/spreadsheets/d/...', isDense: true, border: OutlineInputBorder()), maxLines: 2),
      ]), actions: [
  ...
  List<Map<String, dynamic>> _fb() => [
    {'br':'Michelin','pt':'Primacy 4+','w':225,'a':55,'ri':17,'bp':650,'sp':950,'st':15,'su':'','de':'旗艦舒適呔'},
    {'br':'Bridgestone','pt':'Turanza T005','w':205,'a':55,'ri':16,'bp':520,'sp':780,'st':12,'su':'合誠輪呔','de':'舒適寧靜'},
  ];
  ...
  @override Widget build(BuildContext context) {
    if (!_loaded) return Scaffold(
      ...
      body: Center(child: _err!=null
        ? Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize:MainAxisSize.min, children:[
            ...
            SizedBox(height:16), Text(_err!,textAlign:TextAlign.center,style:TextStyle(color:Colors.red)),
            SizedBox(height:16),
            FilledButton.icon(onPressed:_pickFile,icon:Icon(Icons.file_upload),label:Text('上傳 stock.xlsx')),
            SizedBox(height:8),
            OutlinedButton.icon(onPressed:_setSheetUrl,icon:Icon(Icons.link),label:Text('或連結 Google Sheet')),
          ]))
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('呔妹輪胎'), actions: [
        ...
        IconButton(icon:Icon(Icons.file_upload),tooltip:'上傳 stock.xlsx',onPressed:_pickFile),
        IconButton(icon:Icon(Icons.link),tooltip:'Google Sheet',onPressed:_setSheetUrl),
        IconButton(icon:Icon(Icons.refresh),tooltip:'重新載入',onPressed:_reloadAll),
        Padding(padding:EdgeInsets.symmetric(horizontal:8),child:Text('${_all.length}款 共${_all.fold(0,(s,t)=>s+(t['st']as int))}條',style:Theme.of(context).textTheme.bodySmall)),
      ]),
      body: Column(children:[
        ...
        Padding(padding:EdgeInsets.symmetric(horizontal:12,vertical:6),child:Row(children:[
          Expanded(child:DropdownButtonFormField<String>(value:_brandF.isEmpty?'':_brandF,decoration:InputDecoration(labelText:'品牌',isDense:true,contentPadding:EdgeInsets.symmetric(horizontal:8,vertical:8)),items:[DropdownMenuItem(value:'',child:Text('全部'))].followedBy(brands.map((b)=>DropdownMenuItem(value:b,child:Text(b)))).toList(),onChanged:(v){_brandF=v??'';_apply();})),
          SizedBox(width:8),
        ...
        Expanded(child:_filtered.isEmpty?Center(child:Text('無匹配')):ListView.builder(itemCount:_filtered.length,itemBuilder:(_,i){
          final t=_filtered[i]; return Card(margin:EdgeInsets.symmetric(horizontal:12,vertical:3),child:ListTile(
            title:Text('${t['br']} ${t['pt']}',style:TextStyle(fontWeight:FontWeight.w500)),
            subtitle:Text('${_size(t)}  入\$${t['bp']?.toInt()}  出\$${t['sp']?.toInt()}  利\$${((t['sp']??0)-(t['bp']??0)).toInt()}'),
            trailing:Row(mainAxisSize:MainAxisSize.min,children:[Text('${t['st']}',style:TextStyle(fontSize:18,fontWeight:FontWeight.bold,color:(t['st']as int)<10?Colors.red:Theme.of(context).colorScheme.primary)),SizedBox(width:4),Text('條',style:Theme.of(context).textTheme.bodySmall)]),
            onTap:()=>_edit(t),
          ));
        })),
      ]),
  ...
  Future<Map<String,dynamic>?>_dialog(Map<String,dynamic>?t,bool isNew)async{
    final bc=TextEditingController(text:t?['br']??'');final pc=TextEditingController(text:t?['pt']??'');
    final sc=TextEditingController(text:'${t?['st']??0}');final bpc=TextEditingController(text:'${t?['bp']??0}');final spc=TextEditingController(text:'${t?['sp']??0}');
    final suc=TextEditingController(text:t?['su']??'');final dc=TextEditingController(text:t?['de']??'');
    ...
    final r=await showDialog<Map<String,dynamic>>(context:context,builder:(ctx)=>AlertDialog(
      ...
      content:Form(key:fk,child:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        Row(children:[Expanded(child:TextFormField(controller:bpc,decoration:InputDecoration(labelText:'入貨價 \$',isDense:true),keyboardType:TextInputType.number)),SizedBox(width:8),Expanded(child:TextFormField(controller:spc,decoration:InputDecoration(labelText:'賣出價 \$',isDense:true),keyboardType:TextInputType.number))]),
        TextFormField(controller:suc,decoration:InputDecoration(labelText:'供應商',isDense:true)),
        TextFormField(controller:dc,decoration:InputDecoration(labelText:'描述',isDense:true),maxLines:2),
      ...
      ]))),actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:Text('取消')),FilledButton(onPressed:(){if(fk.currentState?.validate()!=true)return;Navigator.pop(ctx,{'br':bc.text,'pt':pc.text,'w':w,'a':a,'ri':ri,'bp':double.tryParse(bpc.text)??0,'sp':double.tryParse(spc.text)??0,'st':int.tryParse(sc.text)??0,'su':suc.text,'de':dc.text});},child:Text(isNew?'新增':'儲存'))]));
    bc.dispose();pc.dispose();sc.dispose();bpc.dispose();spc.dispose();suc.dispose();dc.dispose();
    return r;
  }
}
...
class _FitmentPageState extends State<FitmentPage> {
  ...
  List<Map<String, dynamic>>? _gens() {
    if (_model == null || _models == null) return null;
    final model = _models!.firstWhere((m) => m['name'] == _model);
    final raw = model['g'] ?? model['generations'];
    if (raw == null) return null;
    return (raw as List).cast<Map<String, dynamic>>().where((g) {
      final n = (g['name'] ?? g['n'] ?? '').toString().trim();
      return n.isNotEmpty && !n.startsWith('---');
    }).map((g) => {
      'name': g['name'] ?? g['n'] ?? '', 'year': g['year'] ?? g['y'] ?? '',
      'pcd': g['pcd'] ?? g['p'] ?? '', 'offset': g['offset'] ?? g['o'] ?? '',
      'cb': g['cb'] ?? g['c'] ?? '', 'thread': g['thread'] ?? g['t'] ?? '', 'torque': g['torque'] ?? g['q'] ?? '',
    }).toList();
  }

  List<String> _ts(Map m) => ((m['ts'] ?? m['tyreSizes'] ?? []) as List).cast<String>();
  List<String> _rs(Map m) => ((m['rs'] ?? m['rimSizes'] ?? []) as List).cast<String>();
  ...
  void _look() {
    if (_make == null || _model == null || _gen == null) return;
    final gens = _gens()!; final gen = gens[_gen!];
    final model = _models!.firstWhere((m) => m['name'] == _model);
    setState(() => _result = {'gen': gen, 'ts': _ts(model), 'rs': _rs(model)});
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        DropdownButtonFormField<String>(value: _make, decoration: const InputDecoration(labelText: '品牌', border: OutlineInputBorder(), isDense: true), items: _makes.map((m) => DropdownMenuItem(value: m, child: Text(m))).toList(), onChanged: _onMake),
        SizedBox(height: 8),
        DropdownButtonFormField<String>(value: _model, decoration: const InputDecoration(labelText: '型號', border: OutlineInputBorder(), isDense: true), items: _models?.map((m) => DropdownMenuItem(value: m['name']?.toString(), child: Text(m['name']?.toString()??''))).toList() ?? [], onChanged: _make != null ? _onModel : null),
        SizedBox(height: 8),
        ...
        if (_result != null) ...[
          ...
          _sr('PCD', _result!['gen']['pcd']), _sr('Offset', _result!['gen']['offset']),
          _sr('Centre Bore', _result!['gen']['cb']), _sr('Thread', _result!['gen']['thread']), _sr('Torque', _result!['gen']['torque']),
          SizedBox(height: 12),
          if ((_result!['ts'] as List).isNotEmpty) ...[
            Text('適用輪胎尺寸', style: const TextStyle(fontWeight: FontWeight.w600)),
            ...(_result!['ts'] as List).map((s) => Card(margin: EdgeInsets.symmetric(vertical: 2), child: ListTile(dense: true, title: Text('$s'), trailing: Icon(Icons.check_circle, color: Colors.green.shade600)))),
          ],
          if ((_result!['rs'] as List).isNotEmpty) ...[
            SizedBox(height: 8), Text('適用鈴尺寸', style: const TextStyle(fontWeight: FontWeight.w600)),
            ...(_result!['rs'] as List).map((s) => Card(margin: EdgeInsets.symmetric(vertical: 2), child: ListTile(dense: true, title: Text(s)))),
          ],
        ],
      ])),
    );
  }
  Widget _sr(String l, String? v) => Padding(padding: EdgeInsets.symmetric(vertical: 2), child: Row(children: [SizedBox(width: 110, child: Text(l, style: const TextStyle(fontWeight: FontWeight.w500))), Text(v ?? '-')]));
}
'''
...
print(f"Lines: {sum(1 for _ in open(path, encoding='utf-8'))}")
